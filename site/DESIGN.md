...
| Route | File | Notes |
|-------|------|--------|
| `/` | `src/pages/index.astro` | Intro. One line per feature. Two choices for adding Atlas, from `applyMethods` in `src/lib/site.ts`. Install opens `/install`. Download opens `/download`. |
| `/install` | `src/pages/install.astro` | Package install only. If the project has no DESIGN.md yet, the next step is the design systems page. Exit is Download a project folder. |
| `/download` | `src/pages/download/index.astro` | Folder download only. Exit is Install the package. |
...
Skills live in `skills/<name>/SKILL.md` at the repo root. `name` and `description` are the disclosure layer. `metadata.updated` (a quoted `YYYY-MM-DD`) and `metadata.updatedBy` are provenance. Change `updated` when the rule set changes. The skill page reads those fields. `src/lib/skill-catalog.ts` holds `title`, `section`, and `order` for the site, keyed by `name`. A section’s place is the lowest order among its skills. The site builds `/{section}/{name}`, so `ux-constitution` in the foundations section publishes at `/foundations/ux-constitution`. The skill sidenav reads that catalog on skill pages only. The homepage has no sidenav. `applyMethods` lists ways to put Atlas in a project. Each method has its own page. One row is `primary`. The package row is that row. It opens `/install`. The folder row opens `/download`. Install writes a pointer into the project's `AGENTS.md` and `CLAUDE.md`. The rules it points at are `instructions/AGENTS.md` in the package, not this repo's `AGENTS.md`. The folder section states the costs and where the files go before the download, and that guidance moves to the top after the folder is saved. The zip is built for the chosen system. No design system omits `DESIGN.md` after the person confirms the project already has design-system documentation. It is not this repo's `AGENTS.md`.

Design systems live in `design-systems/<slug>/DESIGN.md` at the repo root. They are not skills. `updated` (a quoted `YYYY-MM-DD`) and `updatedBy` in that file are provenance. Change `updated` when the public sources are rechecked. The page reads those fields. `src/lib/design-system-catalog.ts` holds `title`, `job`, and `order`. The design-system sidenav is only on those pages. Download serves the file as `DESIGN.md`. Header, burger, and footer read `nav` in `src/lib/site.ts`. Install the package in that list opens `/install`. `to: 'first-skill'` opens the skill with the lowest order. The Design systems item is omitted when that folder is empty.
